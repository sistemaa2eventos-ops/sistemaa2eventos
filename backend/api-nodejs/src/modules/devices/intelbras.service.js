...
class IntelbrasService extends AccessDevice {
    ...
    async enrollUser(pessoa, fotoBase64) {
        try {
            ...
            logger.info(`📸 [Intelbras] Cadastrando (V2): ${pessoa.nome} (ID Hardware: ${hwUserId}) no IP: ${this.ip}`);
    ...
    async configureEventPush(serverIp, serverPort = 3001) {
        try {
            ...
            const params = {
                ...
                'HTTPClient.Server[0].RegisterUrl': '/api/intelbras/events', // URL de registro/heartbeat
                'HTTPClient.Server[0].EventsUrl': '/api/intelbras/events',   // URL para enviar eventos
