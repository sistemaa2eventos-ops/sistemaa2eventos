...
class WebSocketService {
    ...
    async init(httpServer) {
            ...
            cors: {
                origin: function (origin, callback) {
                    if (isOriginAllowed(origin)) {
                        ...
                        callback(new Error('Bloqueado pelo CORS do WebSocket'));
                    }
                },
                methods: ["GET", "POST"]
            }
        ...
        if (redisUrl || process.env.USE_REDIS === 'true') {
            try {
                this.pubClient = createClient({ url: redisUrl || 'redis://localhost:6379' });
                ...
                logger.error('⚠️ Falha ao conectar Sockets no Redis. Caindo para memória local:', error.message);
            }
        } else {
    ...
    emit(event, data, room = null) {
        ...
        if (room) {
            this.io.to(`evento_${room}`).emit(event, data);
            logger.info(`📡 [WS] Emitido '${event}' para sala evento_${room}`);
        } else {
