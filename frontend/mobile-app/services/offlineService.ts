...
export const offlineService = {
    ...
    async savePessoas(pessoas: any[]) {
        const database = await this.getDB();

        await database.withTransactionAsync(async () => {
            // Limpa tabela antes de novo sync massivo para evitar sujeira. 
            // Em produção pode ser otimizado para upsert delta.
            await database.runAsync(`DELETE FROM pessoas;`);

            for (const p of pessoas) {
                await database.runAsync(
                    `INSERT INTO pessoas (id, nome, cpf, qr_code, barcode, rfid_tag, status_acesso, evento_id, updated_at) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    [p.id, p.nome, p.cpf, p.qr_code, p.barcode, p.rfid_tag, p.status_acesso, p.evento_id, p.updated_at]
                );
            }
        });
        console.log(`✅ ${pessoas.length} pessoas armazenadas no cache offline.`);
    },
